...
int cp_index_body(){ //разбор модификаций ссылок на параметры константного пула
	...
	if (current_token == IDENTIFIER) {
		if (strcmp(yytext, "int") == 0) {
			...
			current_token = yylex();
			if (current_token == CST_STRING) return cp_add_utf8(yytext);
		} else if (strcmp(yytext, "str") == 0) {
			current_token = yylex();
			if (current_token == CST_STRING) return cp_add_string(yytext);
		} else if (strcmp(yytext, "class") == 0) {
			current_token = yylex();
			if (current_token == CST_STRING) return cp_add_class(yytext);
			...
			if (current_token == CST_STRING){
				...
				if (current_token == COMMA) {
					current_token = yylex();
					if (current_token == CST_STRING) return cp_add_nameandtype(p1, yytext);
				}
			}
			...
			if (current_token == CST_STRING){
				...
				if (current_token == COMMA) {
					...
					if (current_token == CST_STRING) {
						...
						if (current_token == COMMA) {
							current_token = yylex();
							if (current_token == CST_STRING) return cp_add_fieldref(p1,p2,yytext);
						}
					}
				}
			}
			...
			if (current_token == CST_STRING){
				...
				if (current_token == COMMA) {
					...
					if (current_token == CST_STRING) {
						...
						if (current_token == COMMA) {
							current_token = yylex();
							if (current_token == CST_STRING) return cp_add_methodref(p1,p2,yytext);
						}
					}
				}
			}
