...
/*
	...
	<inline ('type', 'param1', 'param2', 'param3')
...
void RD_inline_body(block *current_block)
{
	...
	if (current_token == OPEN_BR) {
		...
		if (current_token == CST_STRING) {
			if (strcmp(yytext, "getstatic") == 0) ty=1;		//B2 2,получение статического пол€ класса
			if (strcmp(yytext, "putstatic") == 0) ty=2;		//B3 2,установка статического пол€ в классе
			if (strcmp(yytext, "getfield") == 0) ty=3;			//B4 2,перенос пол€ из объекта
			if (strcmp(yytext, "putfield") == 0) ty=4;			//B5 2,установка пол€ в объекте
			if (strcmp(yytext, "invokevirtual") == 0) ty=5;	//B6 2,вызывает метод экземпл€ра, основыва€сь на типе времени выполнени€}
			if (strcmp(yytext, "invokenonvirtual") == 0) ty=6;	//B7 2,вызывает метод экземпл€ра, основыва€сь на не виртуальном типе}
			if (strcmp(yytext, "invokestatic") == 0) ty=7;		//B8 2,вызов метода класса (статического метода)}
			//if (strcmp(yytext, "invokeinterface") == 0) ty=8;	//B9 4,вызывает метод интерфейса}
			///////
			current_token = yylex();
			if (current_token == COMMA) {
				current_token = yylex();
				if (current_token == CST_STRING) {
					strcpy(&param1,yytext);
					current_token = yylex();
					if (current_token == COMMA) {
						current_token = yylex();
						if (current_token == CST_STRING) {
							strcpy(&param2,yytext);
							current_token = yylex();
							if (current_token == COMMA) {
								current_token = yylex();
								if (current_token == CST_STRING) {
									strcpy(&param3,yytext);
									current_token = yylex();
									if (current_token == CLOSE_BR) {
										switch (ty)
										{
										case 1:
											{
												cp_index = cp_add_fieldref(&param1,&param2,&param3);
												bytecode_append(current_block->code, getstatic$);
										...
										case 2:
											{
												cp_index = cp_add_fieldref(&param1,&param2,&param3);
												bytecode_append(current_block->code, putstatic$);
												bytecode_append_short_int(current_block->code, cp_index);
												ty=255;
												break;
											}
										...
										case 4:
											{
												cp_index = cp_add_fieldref(&param1,&param2,&param3);
												bytecode_append(current_block->code, putfield$);
												bytecode_append_short_int(current_block->code, cp_index);
												ty=255;
												break;
											}
										case 5:
											{
												cp_index = cp_add_methodref(&param1,&param2,&param3);
										...
										case 8:
											{
												cp_index = cp_add_methodref(&param1,&param2,&param3);
												bytecode_append(current_block->code, invokeinterface$);
												bytecode_append_short_int(current_block->code, cp_index);
												ty=255;
												break;
											}
	...
	current_token = yylex();
	if (ty !=255) {
		add_error_message(MSG_422, linenum, "inline", "");  //YYTEXT_STRING);
		while ((current_token != SEMI_COLON)
			&& (current_token != END_OF_INPUT))
		{
			current_token = yylex();
