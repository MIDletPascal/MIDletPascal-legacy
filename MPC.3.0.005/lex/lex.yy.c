...
string *build_configuration_define;
...
void yyrestart(FILE *new_file){  //open source file
	yyin=new_file;
	charcount=0;
...
void yyunput(int c, char buf_ptr){ //put the char back to stream
	charbuf[charcount]=c;
	charcount++;
};
	    ...
	    *p='\0';
		...
		if (((c>='A') && (c<='Z')) || ((c>='a') && (c<='z')) || (c=='_')) {
			...
			*p='\0';
			...
			if (c=='.') {
				c=nextchar();
				if (c=='.') {yyunput(c,NULL);yyunput(c,NULL);return CST_INTEGER;}
				i=1;
				j=0;
		        ...
		        yyunput(c,NULL);
				real_constant=real_constant+(j/i);
				//*p='\0';
				return CST_REAL;
			}
			yyunput(c,NULL);
...
rep:
			...
			*p='\0';
			yyunput(c,NULL);
			if (i==0) {p=p-1; char_constant=*p; return CST_CHAR;}
	        ...
	        return END_OF_INPUT; //IDENTIFIER;
		} else {
		    return END_OF_INPUT; //IDENTIFIER;
		}
	} while (r==1);
