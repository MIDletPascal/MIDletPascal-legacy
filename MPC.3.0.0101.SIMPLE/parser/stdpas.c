...
void add_special_function(block *item, enum en_type_class parameter_en, enum en_type_class return_type_en, char *name)
{
	...
	if (parameter_en != void_type) type_list_append(params, param);
	type_destroy(param);
	if (return_type_en == void_type) add_std_procedure(item, name, params);
	else add_std_function(item, name, params, return_type);
}
