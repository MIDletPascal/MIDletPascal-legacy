...
{
	if (code->bytecode_allocated_size > 0) mem_free(code->bytecode);
	mem_free(code);
}
...
void bytecode_append(bytecode *code, char c)
{
	...
	{
		if (code->bytecode_allocated_size == 0) code->bytecode = (char*) mem_alloc(512);
		else code->bytecode = (char*) mem_realloc(code->bytecode, code->bytecode_allocated_size + 512);
		if (code->bytecode == NULL) die(1);
		code->bytecode_allocated_size += 512;
	}
