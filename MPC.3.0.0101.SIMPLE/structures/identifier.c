...
void identifier_destroy(identifier *item)
{
	...
	{
	case constant_name:
		if (item->constant_type->type_class == string_type) string_destroy(item->constant_string_value);
		type_destroy(item->constant_type);
		break;
	case variable_name:
		if (item->variable_type != NULL) type_destroy(item->variable_type);
		break;
	case type_name:
	...
	case procedure_name:
		...
		if (item->variables != NULL) type_list_destroy(item->variables);
		break;		
	case function_name:
...
identifier *identifier_duplicate(identifier *item)
{
	...
	case procedure_name:
		...
		if (item->container_unit != NULL) new_identifier->container_unit = unit_duplicate(item->container_unit);
		else new_identifier->container_unit = NULL;
		break;
	...
	if (item->unit_block != NULL) new_identifier->unit_block = unit_duplicate(item->unit_block);
	else new_identifier->unit_block = NULL;
	return new_identifier;
}
