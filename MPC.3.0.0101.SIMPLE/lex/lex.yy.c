...
//canvasType: NORMAL,FULL_NOKIA,FULL_MIDP20
...
void yyrestart(char *fname) {  //FILE *new_file){  //open source file
	...
	charcount=0;
    string_constant = (string *)&strconst;
    yytext = (char *)&textbuf;
	linenum=1;
};
...
rep:					*p=c;
			...
			*p='\0';
			...
			if (c=='$') {
				...
				if (c=='C') { //display switch
					c=nextchar();
					if (c=='-') canvasType = 0; //NORMAL;
					if (c=='+') canvasType = 1; //FULL_MIDP20;
					if (c=='*') canvasType = 2; //FULL_NOKIA;
				}
			}
			while (c!='}') c=nextchar();
			//goto lab;
