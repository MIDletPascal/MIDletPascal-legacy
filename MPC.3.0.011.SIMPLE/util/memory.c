...
void mem_close()
{
	...
	{		
		...
		it = it->next;
#if SLOWLIST
		if (old_object->data != NULL) free(old_object->data);
		free(old_object);
#else
		mem_free(old_object);
#endif
...
#if SLOWLIST
//fprintf(stdout, "mem_alloc: 002: %d\n\r",s);
	wrapper->data = malloc(s);   // error of memory not:  malloc(s+1) !!!
//fprintf(stdout, "mem_alloc: 0022\n\r");
	...
	if (memory_object_list_end == NULL) die(26);
	return wrapper->data;
}
...
void* mem_realloc(char* old_block, int s)
{
	...
	if ((old_block - sizeof(memory_object)) == new_block) return old_block;
	wrapper = (memory_object*) new_block;
	if (prev != NULL) prev->next = wrapper;
	wrapper->prev = prev;
	if (next != NULL) next->prev = wrapper;
	wrapper->next = next;
	mem_free(old_block);
	return wrapper->data;
#endif
...
{	
	...
	if (next == NULL) memory_object_list_end = prev;
