...
{
	name_table *new_item;
//fprintf(stdout, "name_table_create: 001 [%d]\n\r", sizeof(name_table));
	new_item = (name_table*) mem_alloc(sizeof(name_table)); 	//new_item = mem_alloc(sizeof(name_table));
//fprintf(stdout, "name_table_create: 002\n\r");
	if (new_item == NULL) die(1);
...
{
	if (item == NULL) return;
	if (item->name != NULL) string_destroy(item->name);
	if (item->descriptor != NULL) identifier_destroy(item->descriptor);
	if (item->left_child != NULL) name_table_destroy(item->left_child);
	if (item->right_child != NULL) name_table_destroy(item->right_child);
}
...
name_table *name_table_duplicate(name_table *item)
{
	...
	if (item->left_child != NULL) new_item->left_child = name_table_duplicate(item->left_child);
	else new_item->left_child = NULL;
	if (item->right_child != NULL) new_item->right_child = name_table_duplicate(item->right_child);
	else new_item->right_child = NULL;
	return new_item;
}*/
...
void name_table_insert(name_table *root, string *name, identifier *descriptor)
{
	...
	if (root->name == NULL)
	{
		/* the root item is empty, insert the value into the root item */
		root->name = name;
	...
	do
	{
		...
		if (string_compare < 0)
		{
			if (item->left_child == NULL) break;
			item = item->left_child;
		}
		if (string_compare > 0)
		{
			if (item->right_child == NULL) break;
			item = item->right_child;
		}
	} while (1); /* until the break is called */
//fprintf(stdout, "name_table_insert: 007\n\r");
	...
	root->last = new_item;
//fprintf(stdout, "name_table_insert: 008\n\r");
	if (string_compare < 0) item->left_child = new_item;
	if (string_compare > 0) item->right_child = new_item;
}
...
identifier* name_table_find(name_table *item, string *name)
{
	...
	{
		string_compare = STRING_COMPARE(string_get_cstr(name), string_get_cstr(item->name));
		if (string_compare == 0) return item->descriptor;
		if (string_compare < 0) item = item->left_child;
		if (string_compare > 0) item = item->right_child;
	}
	return NULL;
}
