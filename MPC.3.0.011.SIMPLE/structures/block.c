...
void block_destroy(block *item)
{
	...
	for(i=0; i<item->children_count; i++) block_destroy(item->children[i]);
	if (item->children_count > 0) mem_free(item->children);
...
int block_check_name(block *item, char *cstr)
{
	...
	if (descriptor != NULL)
	{
		if (((descriptor->identifier_class == procedure_name)
			...
			&& (descriptor->forward_declaration == 1)) return_value = 1;
		else return_value = 0;
	}
	/* check the root block's types - those types are protected */
	if (return_value == 1)
	{
		while (item->parent_block != NULL) item = item->parent_block;
		descriptor = name_table_find(item->names, name);
		if ((descriptor != NULL) && (descriptor->identifier_class == type_name)) return_value = 0;
	}
	string_destroy(name);
	return return_value;
}
...
void add_variables(block *item, string_list *identifier_list, type *var_type)
{
	...
	if(item->parent_block != root_block)
	{
		...
		if (block_identifier->variables == NULL) block_identifier->variables = type_list_create();
	}
	else block_identifier = NULL;
	while (it != NULL)
	{
		descriptor = identifier_create();
		...
		if (compiling_unit == 0) initialize_variable(item, descriptor, it->data->cstr, 1, "M");
		else initialize_variable(item, descriptor, it->data->cstr, 1, string_get_cstr(str_program_name));
		if (block_identifier != NULL) type_list_append(block_identifier->variables, descriptor->variable_type);
...
*/
void add_procedure(block *item, string *name, type_list *parameters, int forward_declaration, int linenum)
{
	identifier *descriptor;
	descriptor = name_table_find(item->names, name);
	if (descriptor != NULL)
	{
		if ((forward_declaration != -1) && (descriptor->forward_declaration != -1))
			check_forward_declaration(descriptor, parameters, forward_declaration, NULL);
		if ((descriptor->forward_declaration == -1)	|| (forward_declaration != -1))
...
*/
void add_function(block *item, string *name, type_list *parameters, type *return_type, int forward_declaration, int linenum)
{
	identifier *descriptor;
	descriptor = name_table_find(item->names, name);
	if (descriptor != NULL)
	{
		if ((forward_declaration != -1) && (descriptor->forward_declaration != -1))
			check_forward_declaration(descriptor, parameters, forward_declaration, return_type);
		if ((descriptor->forward_declaration == -1) || (forward_declaration != -1))
			descriptor->forward_declaration = forward_declaration;
	}
	else
...
void load_extern_library(string* library_name)
{
	...
	library_path = string_from_cstr(library_directory);
	string_append(library_path, string_from_cstr("\\Lib_"));
	string_append(library_path, library_name);
	string_append(library_path, string_from_cstr(".class"));
	...
	/* try open the library file from the libraries directory */
	library_file = fopen(library_path->cstr, "rb");
	if (library_file == NULL)
	{
		/* if this fails, try to open an unit file */
		//char *filename = malloc(strlen(output_path) + string_length(library_name) + 10);
		...
		if (symbol_file == NULL)
		{
			if (detect_units_only == 0) add_error_message(448, library_name->cstr, "");
			else goto save_unit_name;
			goto lel_finally;
		}
	}
	...
	if (symbol_file)
	{
		...
		if (detect_units_only == 0) read_symbol_file(symbol_file, library_unit);
		fclose(symbol_file);
	}
	unit_identifier->identifier_class = unit_name;
	unit_identifier->unit_block = library_unit;
	...
	if (library_file)
	{
		...
		{
			int len = (int)strlen(user_libraries);
...
void read_library_file(FILE* library_file, unit* library_unit)
{
	...
	for (i = 1; i<constant_pool_count; i++)
	{
		...
		case 3: /* Integer */
		case 4: /* Float */
			constant_pool[i].param1 = read_long_int(library_file);   //<- long to short ??? 
			break;
		case 5: /* Long */
		case 6: /* double */
			constant_pool[i].param1 = read_long_int(library_file);   //<- long to short ??? 
			constant_pool[i].param2 = read_long_int(library_file);   //<- long to short ??? 
			break;
		case 1: /* Utf8 */
	...
	for(i=0; i<interface_count; i++) read_short_int(library_file);
	...
	for (i = 0; i<methods_count; i++)
	{
		...
		if(method_access_flags == 0x0009)
		{
			...
			{
				if (descriptor[j] == ')') parsing_return_type = 1;
				else if ((descriptor[j] == 'L') && (strncmp(descriptor+j, "Ljava/lang/String;", 18) == 0))
				{
					type* new_type = type_create();
					new_type->type_class = string_type;
					if (!parsing_return_type) type_list_append(method->parameters, new_type);
					else
					{
				...
				else if ((descriptor[j] == 'L') && (strncmp(descriptor+j, "Ljavax/microedition/lcdui/Image;", 32) == 0))
				{
					type* new_type = type_create();
				...
				else if ((descriptor[j] == 'L') && (strncmp(descriptor+j, "Ljavax/microedition/lcdui/Command;", 34) == 0))
				{
					type* new_type = type_create();
				...
				else if ((descriptor[j] == 'L') && (strncmp(descriptor+j, "Ljava/io/InputStream;", 21) == 0))
				{
					type* new_type = type_create();
				...
				else if ((descriptor[j] == 'L') && (strncmp(descriptor+j, "Ljavax/microedition/rms/RecordStore;", 36) == 0))
				{
					type* new_type = type_create();
				...
				else if ((descriptor[j] == 'V') && (parsing_return_type))
				{
...
rlf_finally:
	...
	for (i = 1; i<constant_pool_count; i++) if (constant_pool[i].data != NULL) mem_free(constant_pool[i].data);
...
*/
void add_parameters(block *item, string_list *parameters_list, type *parameters_type, int is_parameter_variable)
{
	string_list *it;
	identifier *descriptor;
	it = parameters_list;
...
*/
void check_forward_declaration(identifier *declaration, type_list *parameters, int forward_declaration, type *return_type)
{
	int different_parameter;
	if ((declaration->identifier_class != procedure_name) && (declaration->identifier_class != function_name))
		return;
	...
	different_parameter = type_list_different_parameter(parameters, declaration->parameters);
	if (different_parameter == -1) add_error_message(403, "", "");
...
void check_unmatched_forward_declarations(block *current_block, name_table *node)
{
	...
	if ((node == NULL) || (node->descriptor == NULL)) return;
...
identifier *get_constant_identifier(block *current_block, char *cstr)
{
	...
	while (current_block != NULL)
	{
		...
		if (declaration != NULL)
		{
			if (declaration->identifier_class == constant_name) found_constant = identifier_duplicate(declaration);	
			break;
		}
		current_block = current_block->parent_block;
	}
	string_destroy(name);
	if (found_constant == NULL) found_constant = identifier_create();
...
identifier *get_identifier(block *current_block, char *cstr)
{
	...
	while (current_block != NULL)
	{
		...
		if (declaration != NULL)
		{
			...
			if (current_block->parent_block == root_block) found_identifier->belongs_to_program_block = 1;
			else found_identifier->belongs_to_program_block = 0;			
			break;
		}
		current_block = current_block->parent_block;
	}
	string_destroy(name);
	if (found_identifier != NULL) return found_identifier;
	/*
			...
			if (descriptor->identifier_class == unit_name)
			{
				...
				*/
				...
				if (unit_table_it->name != NULL)
				{
					...
					{
						if (STRING_COMPARE(string_get_cstr(unit_table_it->name), cstr) == 0)
						{
...
void initialize_variable(block *item, identifier *variable, char *name, int is_static, char *class_name)
{
	...
	if (variable->belongs_to_program_block) storeIntoField = 1;
	...
	case array_type:
		{
			...
			while (it != NULL)
			{
				if (it->data != NULL)
				{
					...
					{
						int cp_index = cp_add_integer(size);
						if (cp_index <= 255)
						{
							bytecode_append(item->code, ldc$);
							bytecode_append(item->code, (char) cp_index);
						}
						else
						{
							bytecode_append(item->code, ldc_w$);
							bytecode_append_short_int(item->code, (short) cp_index);
						}
				...
				|| (variable->variable_type->element_type->type_class == command_type))
			{
				...
				if (variable->variable_type->element_type->type_class == real_type) usesFloat = usesFloat | mathType; //usesFloat = 1;
				...
				for(c=0; c<num; c++)
				{
					...
					if (c < (num-1)) bytecode_append(item->code, aaload$);
				}
				/* new */				
				...
				while (it != NULL)
				{
					if (it->data != NULL)
					{
						...
						bytecode_append(item->code, iaload$);
						bytecode_append(item->code, sipush$);
						bytecode_append_short_int(item->code, (short)(it->data->last_element-it->data->first_element));
		...
		&& (variable->variable_type->type_class != array_type)) /* array initialization stores the variable */
	{
		...
		if (is_static) bytecode_append(item->code, putstatic$);
		else bytecode_append(item->code, putfield$);
...
void transform_break_stmts(bytecode *code, int start_offset, int end_offset, int jump_pos)
{
	...
	while (pos < end_offset)
	{
		if (((unsigned char)(code->bytecode[pos]) == (unsigned char)break_stmt$)
			&& ((pos + 2) < end_offset))
		{
			if ((code->bytecode[pos+1] == 0) && (code->bytecode[pos+2] == 0))
			{
...
void create_class_file(block *program_block, FILE *fp)
{
	...
	for(i=0; i<program_block->children_count; i++) write_method(program_block->children[i], NULL);
	...
	for(i=0; i<program_block->children_count; i++) write_method(program_block->children[i], fp);
...
void write_method(block *current_block, FILE *fp)
{
	...
	if(current_block->parent_block == root_block)
	{
		...
		/* write name index */
		if (compiling_unit == 0) write_short_int(fp, cp_add_utf8("R"));
		else write_short_int(fp, cp_add_utf8("<clinit>"));
	...
	else
	{
		...
		int pos = 1;
		type_list *it;
		descriptor = (char*) mem_alloc(1024*5);
		if (descriptor == NULL) die(1);
		descriptor[0] = '(';
...
//PREVERIFY	map_list = preverify_bytecode(current_block->code, block_identifier);
	...
	if (fp != NULL) code_attribute_length_offset = ftell(fp);
	...
	if (current_block->parent_block == root_block) write_short_int(fp, 0); /* max locals for program block */
	else write_short_int(fp, current_block->next_variable_index + 2); /* max locals */
	write_long_int(fp, current_block->code->bytecode_pos); /* code length */
	if(fp != NULL) fwrite(current_block->code->bytecode, current_block->code->bytecode_pos, 1, fp);
	write_short_int(fp, 0);
	if (map_list != NULL)
	{
/*
		...
		if (fp != NULL) stack_map_attribute_offset = ftell(fp);
		...
		do
		{
			...
			if (map != NULL) write_short_int(fp, (short)(map->bytecode_offset));
			stack_map_size += 2;
			// write the locals
			if (current_block->parent_block == root_block)
			{
			...
			{
				if (fp != NULL) fwrite(&(map->stack[i]->item_type), 1, 1, fp);
		...
		// write the stack map size and the number of entries and the code attribute length
		if (fp != NULL)
		{
			tmp_offset = ftell(fp);
...
{
	char descriptor[128];
	if ((names->descriptor != NULL) && (names->descriptor->identifier_class == variable_name))
	{
		/* access flags: ACC_PUBLIC and ACC_STATIC */
		write_short_int(fp, 0x0001 | 0x0008);
		/* write the name index */
	...
	if ((names->descriptor != NULL) && (names->left_child != NULL))
		write_block_fields(names->left_child, fp);
	/* write the right child */
	if ((names->descriptor != NULL) && (names->right_child != NULL))
