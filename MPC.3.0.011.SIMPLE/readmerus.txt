Модифицированный компилятор MIDletPascal.  v3.0.011  от  03.08.10
Компиляция:
1. Запустить "run.bat"
...
указание_типа_записи: int / utf8 / str / class / nametype / field / method / interface
...
команды использующие переход могут записываться как с относительным смещением, так и с указанием метки.
Метка в ассемблере может быть только числом, название указывается после двоеточия,
напр :123; или в hex-виде :$123abc; после названия метки обязательно ставить символ точка с запятой.
примеры команд
goto :100; - перейти на метку с именем 100
goto 100; - перейти по относительному смещению на 100 байтов вперед (если смещение отрицательное, то необходимо представлять в виде $FFFF,FFFE, и тд.)
