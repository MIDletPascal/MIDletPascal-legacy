...
/* A macro to explicitly declare we didn't forget break inside the case statement */
...
void parser_start()
{
	...
	{
		RD_uses_list();
		if (str_program_name == NULL) str_program_name = string_from_cstr("");
	...
	else
	{
		...
		if (current_token != KWD_END) add_error_message(203, "end", yytext);
	...
	{
		add_error_message(200, ".", yytext);
		/* Error-recovery: find the end of input */
		while (current_token != END_OF_INPUT) current_token = yylex();
	}
	else current_token = yylex();
	if (current_token != END_OF_INPUT) add_warning_message(201, yytext, ""); ///!!!!!!!!!!
	if (error_count == 0)
	{
		FILE *fp;
...
string* RD_program_header()
{
	...
	if ((current_token == KWD_PROGRAM) || (current_token == KWD_UNIT))
	{
		if (current_token == KWD_UNIT) compiling_unit = 1;
		current_token = yylex();
		if (current_token != IDENTIFIER) add_error_message(202, "", "");
		else program_name = string_from_cstr(yytext);
		if ((compiling_unit) && (string_length(program_name) < 3)) add_error_message(454, "", "");
		current_token = yylex();
		if (current_token != SEMI_COLON)
		{
			...
			*/
			while ((current_token != KWD_CONST)
				...
				&& (current_token != DOT)
				&& (current_token != END_OF_INPUT)) current_token = yylex();
		}
		else current_token = yylex();
	}
	return program_name;
...
void RD_unit_interface(block *current_block)
{
	...
	{
		if ((current_token == KWD_IMPLEMENTATION) || (current_token == KWD_INITIALIZATION)) return;
		add_error_message(203, "interface", yytext);
		do
		{
			current_token = yylex();
		}
	...
	current_token = yylex();
	if (current_token == SEMI_COLON) current_token = yylex();
...
{
	if (current_token == END_OF_INPUT) return;
	if (current_token != KWD_IMPLEMENTATION)
	{
		add_error_message(203, "implementation", yytext);
		do
		{
			current_token = yylex();
		}
...
{
	if (current_token != KWD_INITIALIZATION) return; 
	current_token = yylex();
	if (current_token == SEMI_COLON) current_token = yylex();
...
{
	if (current_token != KWD_FINALIZATION) return;
	...
	while (current_token == KWD_USES)
	{
		do {
			...
			if (current_token != IDENTIFIER)
			{
				...
				/* do error recovery */
				while ((current_token != SEMI_COLON) && (current_token != END_OF_INPUT)) current_token = yylex();
				return; //^-???
			}
			/* process the library */
			load_extern_library(string_from_cstr(yytext));
			current_token = yylex();
...
block* RD_block(block *current_block)
{
	...
	case KWD_PROCEDURE:
		{
			if (current_block->parent_block != root_block) add_error_message(431, "", "");
			current_token = yylex();
	...
	default:
		{
			if ((inside_interface_part)
				...
				|| (current_token == KWD_END))) return current_block;
...
void RD_const_declaration(block *current_block)
{
	...
	do
	{
		...
		if (current_token != IDENTIFIER) break;
		if (!block_check_name(current_block, yytext)) add_error_message(400, yytext, "");
		constant_name = string_from_cstr(yytext);
		current_token = yylex();
		...
		case CST_INTEGER:
			{				
				add_integer_constant(current_block, integer_constant, string_get_cstr(constant_name));
				if (inside_interface_part) bsf_write_integer_constant(integer_constant, string_get_cstr(constant_name));
				current_token = yylex();
				break;
			}
		...
		case CST_REAL:
			{
				add_real_constant(current_block, real_constant, string_get_cstr(constant_name));
				if (inside_interface_part) bsf_write_real_constant(real_constant, string_get_cstr(constant_name));
				current_token = yylex();
				break;
			}
		...
		case CST_BOOLEAN:
			{
				add_boolean_constant(current_block, boolean_constant, string_get_cstr(constant_name));
				if (inside_interface_part) bsf_write_boolean_constant(boolean_constant, string_get_cstr(constant_name));
				current_token = yylex();
				break;
			}
		...
		case CST_CHAR:
			{
				add_char_constant(current_block, char_constant, string_get_cstr(constant_name));
				if (inside_interface_part) bsf_write_char_constant(char_constant, string_get_cstr(constant_name));
				current_token = yylex();
				break;
			}
		...
		case CST_STRING:
			{
				add_string_constant(current_block, string_constant, string_get_cstr(constant_name));
				if (inside_interface_part) bsf_write_string_constant(string_constant, string_get_cstr(constant_name));
				current_token = yylex();
				break;
			}
...
void RD_var_declaration(block *current_block)
{
	...
	do
	{
		...
		if (current_token != COLON)
		{
			...
			*/
			while ( (current_token != KWD_CONST) 
				...
				&& (current_token != DOT)
				&& (current_token != END_OF_INPUT)) current_token = yylex();
			if ((current_token == END_OF_INPUT) || (current_token == DOT))
			{
				add_error_message(207, "", "");
				return;
			}
		...
		if (inside_interface_part) bsf_write_variables(identifier_list, var_type);
...
*/
string_list* RD_identifier_list(block *current_block, int check_valid_names)
{
	...
	if (current_token != IDENTIFIER)
	{
		add_error_message(444, yytext, "");
		/* Error-recovery: find the first : or EOF */
		while ((current_token != COLON) && (current_token != DOT) && (current_token != END_OF_INPUT)) current_token = yylex();
		return return_list;
	}
	if ((check_valid_names) && (!block_check_name(current_block, yytext))) add_error_message(400, yytext, "");
	identifier = string_from_cstr(yytext);
...
void RD_type_declaration(block *current_block)
{
	...
	do
	{
		if (current_token != IDENTIFIER)
		{
			...
			|| (current_token == KWD_IMPLEMENTATION)
			|| (current_token == KWD_INTERFACE)
			|| (current_token == KWD_END))) return;
			add_error_message(202, "", "");
			/* Error-recovery: find the first ; or EOF */
			while ((current_token != SEMI_COLON) && (current_token != DOT) && (current_token != END_OF_INPUT)) current_token = yylex();
			if (current_token == SEMI_COLON) RD_block(current_block);
			return;
		}
		type_name = string_from_cstr(yytext);
		if (!block_check_name(current_block, yytext)) add_error_message(400, yytext, "");
		current_token = yylex();
		if (current_token != OP_EQUAL)
		{
			add_error_message(205, yytext, "");
			/* Error-recovery: find the first ; or EOF */
			while ((current_token != SEMI_COLON) && (current_token != DOT) && (current_token != END_OF_INPUT)) current_token = yylex();
			if (current_token == SEMI_COLON) RD_block(current_block);
		...
		if (inside_interface_part) bsf_write_type(type_name, type_type);
		if (current_token != SEMI_COLON)
		{
			add_error_message(200, ";", yytext);
			/* Error-recovery: find the first ; or EOF */
...
/*
	...
	rule used is:
	<RD_procedure_declaration> -> IDN <RD_param_list> ; <RD_proc_block> ; <RD_block>
*/
void RD_procedure_declaration(block *current_block)
{
	...
	inside_implementation_part = 0;
	if (!block_check_name(current_block, yytext)) add_error_message(400, yytext, "");
	procedure_name = string_from_cstr(yytext);
	if (current_token != IDENTIFIER)
	{
		add_error_message(202, "", "");
	...
	if (STRING_COMPARE(procedure_name->cstr, "run") == 0) add_error_message(433, "", "");
	...
	if (inside_interface_part) bsf_write_procedure(procedure_name, parameters);
	...
	if (current_token != SEMI_COLON)
	{
		if (inside_interface_part == 0) add_error_message(200, ";", yytext);
		/* Error-recovery: do nothing */
	}
	else current_token = yylex();
...
/*
	...
	rule used is:
	<RD_function_declaration> -> IDN <RD_param_list> : IDN ; <RD_proc_block> ; <RD_block>
*/
void RD_function_declaration(block *current_block)
{
	...
	inside_implementation_part = 0;
	if (!block_check_name(current_block, yytext)) add_error_message(400, yytext, "");
	function_name = string_from_cstr(yytext);
	if (current_token != IDENTIFIER)
	{
		add_error_message(202, "", "");
		/* Error-recovery: find the first ; */
	...
	if (STRING_COMPARE(function_name->cstr, "run") == 0) add_error_message(433, "", "");
	...
	if (current_token != COLON) add_error_message(200, ":", yytext);
	else current_token = yylex();
	return_type = type_from_name(current_block, yytext);
	if (return_type->type_class == error_type) add_error_message(406, yytext, "");
	...
	if (compiling_unit == 0) initialize_variable(function_block, return_value, NULL, 1, "M");
	else initialize_variable(function_block, return_value, NULL, 1, string_get_cstr(str_program_name));
	identifier_destroy(return_value);
	add_function(current_block, function_name, parameters, return_type, -1, function_linenum);
	forward_declaration = RD_proc_block(function_block, return_type, parameters);	
	add_function(current_block, function_name, parameters, return_type, forward_declaration, function_linenum);
	if (inside_interface_part) bsf_write_function(function_name, parameters, return_type);
	if (current_token != SEMI_COLON)
	{
		if (inside_interface_part == 0) add_error_message(200, ";", yytext);
		/* Error-recovery: do nothing */
...
/*
	...
	Return nonzero value if the declaration is forward declaration.
*/
int RD_proc_block(block *current_block, type *return_type, type_list *parameters)
{
	int forward_declaration = 0;
	if ((current_token == KWD_FORWARD) || (inside_interface_part))
	{
		forward_declaration = 1;
		if (inside_interface_part == 0) current_token = yylex();
		block_destroy(current_block);
	}
	else
	{
		if (inside_interface_part) add_error_message(221, "", "");
		RD_block(current_block);
		...
		if (current_block->parent_block->children == NULL) die(20);
...
/*
	...
	<RD_param_list> -> empty | "(" [var] <RD_identifier_list> : [IDN.]IDN (; [var] <RD_identifier_list> : [IDN.]IDN )+ ")"
*/
type_list* RD_param_list(block *current_block)
{
	...
	if (current_token == OPEN_BR)
	{
		do
		{
			...
			if (current_token == CLOSE_BR) break;
			...
			if (current_token != COLON)
			{
				...
				while ((current_token != CLOSE_BR) && (current_token != END_OF_INPUT)) current_token = yylex(); //!!
				if (current_token == END_OF_INPUT) return parameter_list;
				break;
			}
			else current_token = yylex();
			parameters_type = type_from_name(current_block, yytext);
			if (parameters_type->type_class == error_type)
			{
				/* check if the name is an unit name */
				identifier *unit;
				////!!!!lowercase(yytext);
				unit = get_identifier(current_block, yytext);
				if (unit->identifier_class == unit_name)
				{
					identifier *type_identifier;
					current_token = yylex();
					if (current_token != DOT) add_error_message(200, ".", yytext);
					else current_token = yylex();
					type_identifier = name_table_find(unit->unit_block->names, string_from_cstr(yytext));
					if (type_identifier == NULL) add_error_message(455, yytext, "");
					else parameters_type = type_identifier->defined_type;
				}
			...
			for(i=0; i<len; i++) type_list_append(parameter_list, type_duplicate(parameters_type)); 
			add_parameters(current_block, identifier_list, parameters_type, is_parameter_variable);
			if (current_token != IDENTIFIER)
			{
				add_error_message(202, "", "");
				/* Error-recovery */
				if ((current_token != COMMA) && (current_token != CLOSE_BR) && (current_token != END_OF_INPUT)) current_token = yylex();
			}
			else current_token = yylex();
		} while (current_token == SEMI_COLON);
		if (current_token != CLOSE_BR)
		{
			add_error_message(208, yytext, "");
			/* Error-recovery: find the first ; */
			while ((current_token != CLOSE_BR) && (current_token != END_OF_INPUT)) current_token = yylex();
		}
		current_token = yylex();
...
void RD_block_body(block *current_block)
{
	...
	do
	{
		if (first_pass) first_pass = 0;  //!! заменить на if (current_token == SEMI_COLON) current_token = yylex();
		else current_token = yylex();		
...
void RD_statement(block *current_block)
{
	...
	case KWD_BREAK:
		{
			current_token = yylex();
			if (inside_loop <= 0) add_error_message(219, "", "");
			else
			{
	...
	case KWD_INLINE:
		{
			...
			if (current_token == OPEN_BR) {
				...
				if (current_token != CLOSE_BR) {
					add_error_message(200, "<inline>) ", yytext);
					while ((current_token != SEMI_COLON) && (current_token != END_OF_INPUT)) current_token = yylex();
				} else current_token = yylex();
			} else {
				add_error_message(200, "<inline>(", yytext);
...
int cp_index_body()
{ //разбор модификаций ссылок на параметры константного пула
	char p1[512];
	char p2[512];
	if (current_token == IDENTIFIER)
	{
...
/*
	...
	is:	<RD_if_statement> -> <RD_expression> then <RD_statement> [ else <RD_statement> ]
*/
void RD_if_statement(block *current_block)
{
	...
	/* a simple error-recovery */
	if ((expression_type->type_class != error_type) && (expression_type->type_class != boolean_type)) add_error_message(407, "boolean", "");
	type_destroy(expression_type);
	if (current_token != KWD_THEN) add_error_message(216, yytext, "");
	current_token = yylex();
	/* generate the code */
	...
	bytecode_append(current_block->code, 0);
	RD_statement(current_block);
	//while (current_token == SEMI_COLON) current_token = yylex();  //no C: if then ; else ;
	...
	else
	{
		...
		//no C: if then ; else ;
		/*
		for(i = strlen(yytext) - 1; i >= 0; i--) backchar(yytext[i]);
		backchar(';');
		current_token = yylex();
		*/
	}
...
/*
	...
	<RD_case_statement> -> <RD_expression> OF <RD_case_list> end
*/
void RD_case_statement(block *current_block)
{
	...
	if ((expression_type->type_class != error_type) /* a simple error-recovery */
		...
		&& (expression_type->type_class != string_type)) add_error_message(408, "", "");
	if (current_token != KWD_OF) add_error_message(203, "of", yytext);
	current_token = yylex();
		...
		case IDENTIFIER:
			{
				...
				type *constant_type;
				constant_type = get_constant_type(current_block, yytext);
				if (constant_type->type_class == error_type) add_error_message(410, yytext, "");
				else if (!type_equal(constant_type, case_type))
				{
					string *expected_type_name;
		...
		default:
			{
				add_error_message(206, "", "");
				/* Error-recovery: find the first : */
				while ((current_token != COLON) && (current_token != END_OF_INPUT)) current_token = yylex();
				if (current_token == END_OF_INPUT) return;
				break;
			}
		}
...
void RD_while_statement(block *current_block)
{
	...
	if ((expression_type->type_class != error_type) && (expression_type->type_class != boolean_type)) add_error_message(407, "boolean", "");
	type_destroy(expression_type);
	if (current_token != KWD_DO) add_error_message(203, "do", yytext);
	else current_token = yylex();
	inside_loop++;
...
void RD_repeat_statement(block *current_block)
{
	...
	break_pos2 = current_block->code->bytecode_pos;
	inside_loop --;
	if (current_token != KWD_UNTIL) add_error_message(203, "until", yytext);
	current_token = yylex();
	expression_type = RD_expression(current_block);
	/* generate the code */
	bytecode_append(current_block->code, ifeq$);
...
/*
	The for statement.
	<RD_for_statement> -> IDN := <RD_expression> (to | downto) <RD_expression> do <RD_statement>
*/
void RD_for_statement(block *current_block)
{
	...
	int evaluate_pos;
	int break_pos1, break_pos2;
	iterator_type = get_variable_type(current_block, yytext);
	iterator = get_identifier(current_block, yytext);
	iterator_name = string_from_cstr(yytext);
	...
	if ((iterator_type->type_class != integer_type) && (iterator_type->type_class != char_type)) add_error_message(411, "", "");
	if (current_token != IDENTIFIER)
	{
		add_error_message(202, "", "");
		/* Error-recovery: find the first := */
		while ((current_token != OP_ASSIGN) && (current_token != END_OF_INPUT)) current_token = yylex();
 		if (current_token == END_OF_INPUT)
		{
	...
	if (current_token != OP_ASSIGN) add_error_message(209, yytext, "");
	else current_token = yylex();
	...
	if (direction == 1) bytecode_append(current_block->code, if_icmplt$);
	else bytecode_append(current_block->code, if_icmpgt$);
	bytecode_append_short_int(current_block->code, 0);
	...
	create_variable_bytecode(iterator, current_block->code, iterator_name->cstr, iterator->belongs_to_program_block);
	if (direction == 1) bytecode_append(current_block->code, iconst_1$);
	else bytecode_append(current_block->code, iconst_m1$);
...
type* RD_type(block *current_block)
{
	...
	case KWD_PACKED:
		...
		current_token = yylex();
		if (current_token != KWD_ARRAY) add_error_message(203, "array", yytext);
		//no_break;
	case KWD_ARRAY:
		current_token = yylex();
	...
	case IDENTIFIER:
		{
			identifier* name;
			type *declared_type;
			declared_type = type_from_name(current_block, yytext);
			////!!!!lowercase(yytext);
			name = get_identifier(current_block, yytext);
			if (name->identifier_class == unit_name)
			{
				...
				type_identifier = name_table_find_cstr(name->unit_block->names, yytext);
				if (type_identifier == NULL) add_error_message(455, yytext, "");
				else declared_type = type_duplicate(type_identifier->defined_type);
			}
			...
			else
			{
				...
				constant_identifier  = get_constant_identifier(current_block, yytext);
				type_destroy(declared_type);
				    ...
				    || (constant_identifier->constant_type->type_class == char_type)))
				{
					...
					{
						if (return_type->interval_base_type != integer_type) add_error_message(416, "", "");
						return_type->last_element = integer_constant;
					}
					else if (current_token == CST_CHAR)
					{
						if (return_type->interval_base_type != char_type) add_error_message(416, "", "");
	...
	case CST_CHAR:
		{
			...
			return_type->last_element = return_type->first_element + 1;
			current_token = yylex();
			if (current_token != DOTDOT) add_error_message(213, yytext, "");
			current_token = yylex();
			if ((current_token != IDENTIFIER)
				&& (current_token != CST_INTEGER)
				&& (current_token != CST_CHAR)) add_error_message(217, "", "");
			else
			{
				if (current_token == IDENTIFIER)
				{
					...
					identifier *constant_identifier;
					constant_type = get_constant_type(current_block, yytext);
					if (constant_type->type_class != return_type->interval_base_type) add_error_message(413, "", "");
					type_destroy(constant_type);
					constant_identifier = get_constant_identifier(current_block, yytext);
					if (constant_identifier->identifier_class != constant_name) add_error_message(414, yytext, "");
					else
					{
						if (return_type->interval_base_type == integer_type)
							return_type->last_element = constant_identifier->constant_int_value;
				...
				else if (current_token == CST_INTEGER) return_type->last_element = integer_constant;
				else return_type->last_element = (long int) char_constant;
			}
			current_token = yylex();
			break;
		}
	...
	case OPEN_BR:
		{
			...
			{
				current_token = yylex();
				if (current_token == OPEN_BR) brack_count ++;
				if (current_token == CLOSE_BR) brack_count --;
				if (current_token == END_OF_INPUT) return return_type;
			}
			current_token = yylex();
			break;
		}
	...
	default:
		{
			...
			while ((current_token != CLOSE_SQ_BR)
				&& (current_token != SEMI_COLON)
				&& (current_token != END_OF_INPUT)
				&& (current_token != KWD_END)) current_token = yylex();
			break;
		}
	}
	return return_type;
...
type* RD_array_declaration(block *current_block)
{
	...
	if (current_token != OPEN_SQ_BR) add_error_message(200, "[", yytext);
	do
	{
		current_token = yylex();
		dimension = RD_basic_type(current_block);
		if (dimension->type_class != interval_type) add_error_message(430, "", "");
		type_list_append(new_type->dimensions_list, dimension);
		type_destroy(dimension);
		/*
		if ((current_token != CLOSE_SQ_BR) && (current_token != COMMA)) add_error_message(200, ",", yytext);
		if (current_token == END_OF_INPUT)
		{
			add_error_message(207, "", "");
		...
		*/
	} while (current_token == COMMA); //(current_token != CLOSE_SQ_BR);
	if (current_token != CLOSE_SQ_BR) add_error_message(200, "]", yytext);
	if (current_token == END_OF_INPUT)
	{
		add_error_message(207, "", "");
		type_destroy(new_type);
...
type* RD_record_declaration(block *current_block)
{
	...
	do
	{
		...
		if ((current_token == KWD_END) || (current_token == END_OF_INPUT)) break;
		identifier_list = RD_identifier_list(current_block, 0);
		if (current_token != COLON) add_error_message(200, ":", yytext);
		old_linenum = linenum;
		current_token = yylex();
	...
	if (current_token != KWD_END) add_error_message(214, yytext, "");
	current_token = yylex();
...
type* RD_expression(block *current_block)
{
		...
		|| (current_token == OP_NOT_EQUAL))
	{
		...
		else
		{
			...
			if ((type1->type_class == integer_type) || (type2->type_class == integer_type))
			{
				if (cast != 1)
				{
					//usesFloat = 1;
					usesFloat = usesFloat | mathType;
					if (cast == 3) bytecode_append(current_block->code, swap$);
					if (mathType == 1)
					{
			...
			{
				if (type1->type_class == real_type) usesFloat = usesFloat | mathType; //usesFloat = 1;
				switch (old_token)
				{
				case OP_LESS:
			...
			{
				if (old_token == OP_EQUAL) bytecode_append(current_block->code, if_acmpeq$);
				else if (old_token == OP_NOT_EQUAL) bytecode_append(current_block->code, if_acmpne$);
				else
				{
					new_linenum=old_linenum;
					add_error_message(417, "", "");
...
type* RD_sum(block *current_block)
{
		...
		|| (current_token == OP_XOR))
	{
		...
		else
		{
			...
			/* if operands need to be casted */
			if ((type1->type_class != string_type) && (cast != 1))
			{				
				//usesFloat=1;
				usesFloat = usesFloat | mathType;
				if (cast == 3) bytecode_append(current_block->code, swap$);
		...
		if ((type1->type_class == string_type) && (old_token == OP_PLUS))
		{
			int class_index;
			int init_index;
			int append_index;
...
type* RD_mult(block *current_block)
{
		...
		|| (current_token == OP_SHL))
	{
		...
		if ((type1->type_class == boolean_type) && (old_token != OP_AND))
		{
			new_linenum=old_linenum;
			add_error_message(417, "", "");
		}
		...
		if ((type1->type_class == boolean_type) && (old_token == OP_AND)) bytecode_append(current_block->code, iand$);
	}
	return type1;
}
...
void RD_assignment_or_procedure_call(block *current_block)
{
	...
	name = get_identifier(current_block, yytext);
	element_name = string_from_cstr(yytext);
	...
	{		
		...
		current_token = yylex();
		unit_name = name_table_find(name->unit_block->names, string_from_cstr(yytext));

		//!!!
		//identifier_text = malloc(strlen(yytext) + 1);
		//strcpy(identifier_text, yytext);
		//!!!
		...
		current_token = yylex();
		/*
		if (name->identifier_class == procedure_name) goto procedure_call;
		if (name->identifier_class == variable_name) goto variable_lvalue;
		add_error_message(457, yytext, "");
		*/
		if ((name->identifier_class != procedure_name)
		&& (name->identifier_class != variable_name))
		add_error_message(457, yytext, "");
	}
	...
	{
//procedure_call:
		if (name->standard_function) create_std_function_prefix(current_block->code, element_name->cstr);
		if (type_list_length(name->parameters) == 0)
		{
			...
			if (current_token == OPEN_BR)
			{
				...
				while (br_depth > 0)
				{
					if ((current_token == END_OF_INPUT) || (current_token == KWD_END))
					{
						identifier_destroy(name);
						string_destroy(element_name);
						return;
					...
					current_token = yylex();
					if (current_token == OPEN_BR) br_depth ++;
					if (current_token == CLOSE_BR) br_depth --;
				}
				current_token = yylex();
			}
		}
		...
		{
			if (current_token != OPEN_BR) add_error_message(420, "", "");
			else
			{
				int compare_result;
				int old_linenum;
				...
				if (current_token != CLOSE_BR) add_error_message(200, ")", yytext);
				else current_token = yylex();
			}
		}
		/* create the bytecode */
		if (!name->standard_function)
		{
			...
			type_list *it;
			int pos;
			char* descriptor = (char*) mem_alloc(1024*5);			//old: mem_alloc(5*1024);
			if (descriptor == NULL) die(1);
			descriptor[0] = '(';
			if (!name->unit_function)
			{
				block_identifier = name_table_find(current_block->names, element_name);
				if (block_identifier == NULL) block_identifier = name_table_find(current_block->parent_block->names, element_name);
			}
			else block_identifier = name;
			it = block_identifier->parameters;
			pos = 1;
			while(it != NULL)
			{
				if (it->data == NULL) break;
				get_field_descriptor(it->data, descriptor + pos);
				pos = (int)strlen(descriptor);
				it = it->next;
			}
			descriptor[pos] = ')';
			pos ++;
			descriptor[pos] = '\0';
			if (block_identifier->identifier_class == procedure_name) strcat(descriptor, "V");
			else get_field_descriptor(block_identifier->return_type, descriptor + pos);
			////!!!!lowercase(element_name->cstr);
			if (name->unit_function)
			{
				string *full_unit_name;
				if (name->container_unit->is_library) full_unit_name = string_from_cstr("Lib_");
				else full_unit_name = string_create();
			...
			{
				if (compiling_unit == 0) methodref_index = cp_add_methodref("M", element_name->cstr, descriptor);
				else methodref_index = cp_add_methodref(string_get_cstr(str_program_name), element_name->cstr, descriptor);
			}
	...
	{
		if (STRING_COMPARE(element_name->cstr, current_block->block_name->cstr) != 0) add_error_message(432, element_name->cstr, "");
	}
...
//variable_lvalue:
		...
		if (name->identifier_class == variable_name) current_type = type_duplicate(name->variable_type);
		if (name->identifier_class == parameter_name) current_type = type_duplicate(name->parameter_type);
		if (name->identifier_class == function_name) current_type = type_duplicate(name->return_type);
		/* create the get and put bytecodes */
		is_field = name->belongs_to_program_block;
		if (name->identifier_class == function_name) is_field = 0;
		...
		while (current_token != OP_ASSIGN)
		{
			...
			if (current_token == DOT)
			{
				...
				new_type = type_find_record_element(current_type, yytext);
				if (new_type == NULL) add_error_message(447, yytext, "");
				else
				{
					...
					////!!!!lowercase(yytext);
					fieldref_index = cp_add_fieldref(record_name, yytext, field_descriptor);
			...
			else if (current_token == OPEN_SQ_BR)
			{
				...
				case real_type:
					if (mathType == 1) bytecode_append(put_bytecode, iastore$);
					else bytecode_append(put_bytecode, aastore$);
					break;
		...
		current_token = yylex();
		expression_type = RD_expression(current_block);
		if ((type_equal_cast(current_type, expression_type) != 1) && (current_type->type_class == real_type))
		{
			//usesFloat = 1;
			usesFloat = usesFloat | mathType;
			if (mathType == 1)
		...
		if ((type_equal_cast(current_type, expression_type) == 2) && (current_type->type_class == string_type))
		{
			int method_index = cp_add_methodref("java/lang/String", "valueOf", "(C)Ljava/lang/String;");
			/* string cast */
			bytecode_append(current_block->code, invokestatic$);
		...
		if ((type_equal_cast(current_type, expression_type) != 1) && (type_equal_cast(current_type, expression_type) != 2))
		{
			new_linenum=assign_linenum;
			add_error_message(423, "", "");
		}
		else if (current_type->type_class == array_type)
		{
			new_linenum=assign_linenum;
	...
	if (name != NULL) identifier_destroy(name);
	if (element_name != NULL) string_destroy(element_name);
}
...
type* RD_value(block *current_block)
{
	...
	if (current_token == CST_INTEGER)
	{
		...
		default:
			{
				if ((integer_constant >=-128) && (integer_constant <= 127)) //if (abs(integer_constant) <= 127)   //!! "<127"
				{
					bytecode_append(current_block->code, bipush$);
					bytecode_append(current_block->code, (char)(integer_constant));
				}
				else if ((integer_constant >=-32768) && (integer_constant <= 32767)) //if (abs(integer_constant) <= 32767)   //!! "<16000"
				{
	...
	if (current_token == CST_REAL)
	{
		...
		//cst = real_constant;
		csti = (int)real_constant; //(int)cst;
		cstf = (int)((real_constant - csti)*100000);
		//usesFloat = 1;
		usesFloat = usesFloat | mathType;
		if (mathType == 1)
		{
			/* put the integer part on the stack */
			if ((csti >=-128) && (csti <= 127)) //if (abs(csti) <= 127)  //!!"< 127"
			{
				bytecode_append(current_block->code, bipush$);
				bytecode_append(current_block->code, csti);
			} 
			else if ((csti >=-32768) && (csti <= 32767)) //if (abs(csti) < 32767)  //!!"< 15000"
			{
				bytecode_append(current_block->code, sipush$);
			...
			if ((cstf >=-128) && (cstf <= 127)) //if (abs(cstf) <= 127)  //!!"< 127"
			{
				bytecode_append(current_block->code, bipush$);
				bytecode_append(current_block->code, cstf);
			} 
			else if ((cstf >=-32768) && (cstf <= 32767)) //if (abs(cstf) < 32767)  //!!"< 15000"
			{
				bytecode_append(current_block->code, sipush$);
	...
	{
		if (boolean_constant) bytecode_append(current_block->code, iconst_m1$);
		else bytecode_append(current_block->code, iconst_0$);
		current_token = yylex();
	...
	if (current_token == IDENTIFIER)
	{
		identifier *name;
		char *identifier_text;
		identifier_text = (char*) mem_alloc(strlen(yytext) + 2);
		strcpy(identifier_text, yytext);
...
//constant_value:
		...
		*/
		if (name->identifier_class == unit_name)
		{
			...
			//!!!!
			identifier_text = mem_realloc(identifier_text, strlen(yytext) + 2);  //new!
			//mem_free(identifier_text); identifier_text = malloc(strlen(yytext) + 1);
			//!!!!
			...
			current_token = yylex();
			/*
			if (name->identifier_class == function_name) goto function_call;
			if (name->identifier_class == constant_name) goto constant_value;
			if (name->identifier_class == variable_name) goto variable_value;
			add_error_message(458, yytext, "");
			*/
			if ((name->identifier_class != function_name)
				&& (name->identifier_class != constant_name)
				&& (name->identifier_class != variable_name))
...
//constant_value:
		...
		if ((name->identifier_class == variable_name) || (name->identifier_class == parameter_name))
		{
			type *current_type;
			type *new_type;
			int is_field;
//variable_value:
			...
			identifier_destroy(name);
			name = NULL;
			while ((current_token == DOT) || (current_token == OPEN_SQ_BR))
			{
...
//function_call:
		...
		if (name->identifier_class == function_name)			
		{
			...
			if (name->standard_function) create_std_function_prefix(current_block->code, identifier_text);
			if (type_list_length(name->parameters) == 0)
			{
				int br_depth = 0;
				if (current_token == OPEN_BR)
				...
				{
					string *full_unit_name;
					if (name->container_unit->is_library == 1) full_unit_name = string_from_cstr("Lib_");
					else full_unit_name = string_create();
					string_append(full_unit_name, name->container_unit->unit_name);
					////!!!!lowercase(identifier_text);
					strcpy(method_type, "()");
					get_field_descriptor(name->return_type, method_type + 2);
					method_index = cp_add_methodref(full_unit_name->cstr, identifier_text, method_type);
					bytecode_append(current_block->code, invokestatic$);
					bytecode_append_short_int(current_block->code, method_index);
					string_destroy(full_unit_name);
				}
				else if (!name->standard_function)
				{
					...
					strcpy(method_type, "()");
					get_field_descriptor(name->return_type, method_type + 2);
					if (compiling_unit == 0) methodref_index = cp_add_methodref("M", identifier_text, method_type);
					else methodref_index = cp_add_methodref(string_get_cstr(str_program_name), identifier_text, method_type);
					bytecode_append(current_block->code, invokestatic$); /* call */
					bytecode_append_short_int(current_block->code, methodref_index);
				}
				/* handle standard functions and procedures */
				else create_std_function_code(current_block->code, identifier_text);
				type_destroy(return_type);			
				mem_free(identifier_text);
			...
			else
			{
				...
				int old_linenum;
				type_list *parameter_list;
				if (current_token != OPEN_BR) add_error_message(200, "(", yytext);
				current_token = yylex();
				old_linenum = linenum;
				parameter_list = RD_expression_list_cast(current_block, name->parameters);
				compare_result = type_list_different_parameter_cast(parameter_list, name->parameters);
...
void create_constant_bytecode(identifier *item, bytecode *code)
{
	...
	case real_type:
		{
			...
			csti = (int)item->constant_real_value;  //(int)cst;
			cstf = (int)((item->constant_real_value - csti)*100000);  //cstf = (cst - (int)cst)*100000;
			//usesFloat = 1;
			usesFloat = usesFloat | mathType;
			if (mathType == 1)
			{
	...
	case boolean_type:
		if (item->constant_int_value) bytecode_append(code, iconst_m1$);
		else bytecode_append(code, iconst_0$);
		break;
...
void create_variable_bytecode(identifier *item, bytecode *code, char *name, int is_field)
{
	...
	if ((is_field) || (item->unit_function == 1))
	{
		if (item->identifier_class == variable_name)
		{
			...
			{
				if (compiling_unit == 0) bytecode_append_short_int(code, cp_add_fieldref("M", name, type_descriptor));
				else bytecode_append_short_int(code, cp_add_fieldref(string_get_cstr(str_program_name), name, type_descriptor));
			}
			else
			{
				string *unit_name;
				if (item->container_unit->is_library == 0) unit_name = string_create();
				else unit_name = string_from_cstr("Lib_");
...
void replace_aaload_instruction(bytecode *code, type *element_type)
{
	...
	case real_type:
		if (mathType == 1) bytecode_append(code, iaload$);
		else bytecode_append(code, aaload$);
		break;
...
void adjust_indices(bytecode *code, type *dimension)
{
	...
	default:
		if ((dimension->first_element >=-128) && (dimension->first_element <= 127)) //if (dimension->first_element <= 127)
		{
			bytecode_append(code, bipush$);
			bytecode_append(code, (char)dimension->first_element);
		}
		else if ((dimension->first_element >=-32768) && (dimension->first_element <= 32767))
		{
			bytecode_append(code, sipush$);
			bytecode_append_short_int(code, (short)dimension->first_element);
		}
		else
		{
			bytecode_append(code, ldc_w$);
			bytecode_append_short_int(code, (short)cp_add_integer(dimension->first_element));
		}
	}
	if (dimension->first_element != 0) bytecode_append(code, isub$);
