...
int cp_add_utf8(char *str)
{
	...
	{
		if ((constants[i].tag == 1) && (strcmp(constants[i].data, str) == 0))
	...
	size = len;
	/*
	for (i=0; i<len; i++) if ((unsigned char)str[i] > 127) size += 2; else size ++;
	*/
	entry->data = (char*) mem_alloc(size + 1);
	entry->data_len = size;
	if (entry->data == NULL) die(1);
	strcpy(entry->data, str);
	//for(j=0; j<len; j++) entry->data[j] = str[j];
	/*
	for(i=0, j=0; i<len; i++)
		if ((unsigned char)str[i] <= 127) {entry->data[j] = str[i]; j++;}
		else { entry->data[j] = (char)0xC2 | (str[i]>>7)&0x01; j++; entry->data[j] = (str[i] & 0x3F) | 0x80; j++;}
	entry->data[j] = '\0';
	*/
...
int cp_add_string(char *str)
{
	...
	for(i=0; i<constant_pool_size; i++)
	{
		if ((constants[i].tag == 8) && (constants[i].param1 == utf8_const))
			return i+1;
	}
...
int cp_add_class(char *class_name)
{
	...
	for(i=0; i<constant_pool_size; i++)
	{
		if ((constants[i].tag == 7) && (constants[i].param1 == utf8_const))
			return i+1;
	}
	/* create the new entry */
	entry = cp_create_new_entry();
...
int cp_add_nameandtype(char *name, char *type)
{
	...
	for(i=0; i<constant_pool_size; i++)
	{
		if ((constants[i].tag == 12) && (constants[i].param1 == name_index) && (constants[i].param2 == type_index))
			return i+1;
	}
	/* create the new entry */
	entry = cp_create_new_entry();
...
int cp_add_fieldref(char *class_name, char *name, char *type)
{
	...
	for(i=0; i<constant_pool_size; i++)
	{
		if ((constants[i].tag == 9) && (constants[i].param1 == class_index) && (constants[i].param2 == nameandtype_index))
			return i+1;
	}
	/* create the new entry */
	entry = cp_create_new_entry();
...
/*
	Add the method entry into the constant pool
*/
...
{
	constant_pool_size ++;
	if (constant_pool_size == 1) constants = (constant_pool*) mem_alloc(sizeof(constant_pool));
	else constants = (constant_pool*) mem_realloc(constants, sizeof(constant_pool) * constant_pool_size);
	if (constants == NULL) die(1);
