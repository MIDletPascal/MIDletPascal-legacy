...
//#define compile_write2(x,y) sprintf(x,y);
//#define compile_write2(x,y) sprintf(x,y);
//compile_terminate
char* buildFile;
...
void compile_write2(char* x, char* y){
	//fprintf(stderr, x,y);
	fprintf(stdout,x,y);
};
...
//#define compile_terminate() fprintf(stderr, "compile_terminate\n");
//#define compile_write2(x,y) fprintf(stderr, x,y);
//#define compile_write2(x,y,z) fprintf(stderr, x,y,z);
//#define compile_progress(x) fprintf(stderr, "%i,",x);
	...
	*/
    if (argc >= 2)
	{
		...
		*units=NULL;
		...
		//library_directory=malloc(1);
		//strcpy(library_directory, "" );
		////////
        //build_configuration = string_from_cstr("DeFaUlt");
	    //lowercase(string_get_cstr(build_configuration));
	...
	{
		if (usesForms)
			compile_write2("<<FS.class\n","");
		if (usesSupport)
			compile_write2("<<S.class\n","");
		if (usesFloat == 1)
			compile_write2("<<Float1.class\n","");
		if (usesFloat == 2)
			compile_write2("<<Float2.class\n","");
		if (usesRecordStore == 1)
			compile_write2("<<RS.class\n","");
		if (usesHttp == 1)
			compile_write2("<<H.class\n","");
		if (usesPlayer == 1)
			compile_write2("<<P.class\n","");
		if (usesSMS == 1)
			compile_write2("<<SM.class\n","");
		if (usesRegisteredFeature)
			compile_write2("<<REGISTERED\n","");
	}
	mem_close();
    return 0;
}