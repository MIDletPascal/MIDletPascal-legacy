	...
	while (current_token == KWD_USES)
	{
		do {
			...
			if (current_token != IDENTIFIER)
			{
				...
				/* do error recovery */
				while ((current_token != END_OF_INPUT) && (current_token != SEMI_COLON)) current_token = yylex();
				return; //^-???
			}
			/* process the library */
	...
	if (current_token == OPEN_BR)
	{
		do
		{
			...
			if (current_token != COLON)
			{
				...
				while (current_token != CLOSE_BR) current_token = yylex();
			...
			if (current_token != IDENTIFIER)
			{
				add_error_message(202, "", "");
				/* Error-recovery */
				if ((current_token != COMMA) && (current_token != CLOSE_BR)) current_token = yylex();
			}
			else current_token = yylex();
		} while (current_token == SEMI_COLON);
...
void RD_if_statement(block *current_block)
{
	...
	bytecode_append(current_block->code, 0);
	RD_statement(current_block);
	while (current_token == SEMI_COLON) current_token = yylex();
	...
	case KWD_PACKED:
		{
			...
			if (current_token != KWD_ARRAY)
				add_error_message(203, "array", yytext);
			//no_break;
		}
...
type* RD_expression(block *current_block)
{
		...
		|| (current_token == OP_NOT_EQUAL))
	{
		...
		else
		{
			...
			{
				if (type1->type_class == real_type) usesFloat = 1;
				switch (old_token)
				{
				case OP_LESS:
