...
void load_extern_library(string* library_name)
{
	...
	{
		add_error_message(MSG_450, linenum, string_get_cstr(library_name), "");
		fclose(library_file);
		goto lel_finally;
	}
	...
	if (library_file == NULL)
	{
		...
		if (symbol_file == NULL)
		{
			if (detect_units_only == 0)
				add_error_message(MSG_448, linenum, library_name->cstr, "");
			else
				goto save_unit_name;
			goto lel_finally;
		}
...
void read_library_file(FILE* library_file, unit* library_unit)
{
	...
	{
		add_error_message(MSG_449, linenum, "", "");
		fclose(library_file);
		die(MSG_006);		
		return;
	}
	...
	{
		add_error_message(MSG_451, linenum, "", "");
	}
	...
	for (i = 0; i<methods_count; i++)
	{
		...
		if(method_access_flags == 0x0009)
		{
			...
			for(j = 1; j<constant_pool[method_descriptor_index].data_len; j++)
			{
				...
				{
					add_error_message(MSG_452, linenum, "", "");
					break;
				}
			}
...
void check_unmatched_forward_declarations(block *current_block, name_table *node)
{
	...
	{
		add_error_message(MSG_441, declaration->subprogram_linenum, "", "");
	}
				...
				if (unit_table_it->name != NULL)
				{
					while (unit_table_it != NULL)
					{
							              ...
							              cstr) == 0)
						{
							...
							{
								add_error_message(MSG_459, linenum, cstr, "");
	...
	case array_type:
		{
			...
			while (it != NULL)
			{
				if (it->data != NULL)
				{
					...
					if (size <= 0)
						add_error_message(MSG_437, linenum, "", "");
					...
					{
						add_error_message(MSG_438, linenum, "", "");
					}
				}
				it = it->next;
