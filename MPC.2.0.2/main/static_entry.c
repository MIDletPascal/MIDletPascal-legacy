...
//		compile_write2("Compiling \'%s\'...\n", source_file_name);
	...
	sprintf(final_message, "Done - %d error(s), %d warning(s)\n", error_count, warning_count);
	compile_write2(final_message, "");

	mem_close();


	returnStructure.error_count = error_count;
	returnStructure.warning_count = warning_count;

	returnStructure.usesFloat = usesFloat;
	returnStructure.usesForms = usesForms;
	returnStructure.usesHttp = usesHttp;
	returnStructure.usesPlayer = usesPlayer;
	returnStructure.usesSMS = usesSMS;
	returnStructure.usesRecordStore = usesRecordStore;
	returnStructure.usesRegisteredFeature = usesRegisteredFeature;
	returnStructure.usesSupport = usesSupport;
	returnStructure.userLibraries = user_libraries;
	returnStructure.units = *units;
