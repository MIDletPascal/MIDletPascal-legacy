...
int type_equal(type *type1, type *type2)
{
	...
	case array_type:
		{
			if (!type_equal(type1->element_type, type2->element_type)) return 0;
			if (type_list_different_parameter(type1->dimensions_list, type2->dimensions_list) != 0)
				return 0;
			break;
		}
	...
	case interval_type:
		{
			if (type1->interval_base_type != type2->interval_base_type) return 0;
			if (type1->first_element != type2->first_element) return 0;
			if (type1->last_element != type2->last_element) return 0;
			break;
		}
	}
	return 1;
...
{
	if (type_equal(type1, type2)) return 1;
	if ((type1->type_class == real_type) && (type2->type_class == integer_type)) return 2;
	if ((type1->type_class == integer_type) && (type2->type_class == real_type)) return 3;
	if ((type1->type_class == string_type) && (type2->type_class == char_type)) return 2;
	if ((type1->type_class == char_type) && (type2->type_class == string_type)) return 3;
	return 0;
}
	...
	while (element_names != NULL)
	{
		if (element_names->data != NULL)
		{
			...
			while (item_list != NULL)
			{
				...
				{
					if (STRING_COMPARE(string_get_cstr(item_list->data), string_get_cstr(element_names->data)) == 0)
...
type* type_find_record_element(type *record, char *cstr)
{
	string_list *names;
	type_list *types;
	if (record->type_class != record_type) return NULL;
	names = record->elements_name_list;
	types = record->elements_type_list;
	while (names != NULL)
	{
		if (names->data != NULL) if (STRING_COMPARE(cstr, string_get_cstr(names->data)) == 0)
