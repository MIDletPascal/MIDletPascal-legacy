...
void RD_inline_body(block *current_block)
{
	...
	do {
		...
		{
			if (integer_constant<=255) bytecode_append(current_block->code, integer_constant);
			else if (integer_constant<=65535) bytecode_append_short_int(current_block->code, integer_constant);
...
type* RD_value(block *current_block)
{
	...
	if (current_token == IDENTIFIER)
	{
		identifier *name;
		char *identifier_text;
		identifier_text = (char*) mem_alloc(strlen(yytext) + 1);
		strcpy(identifier_text, yytext);
...
void adjust_indices(bytecode *code, type *dimension)
{
	...
	default:
		if (dimension->first_element <= 127)
		{
			bytecode_append(code, bipush$);
